/**
 * docType.js - 单据权限 model
 * @date: 2018-10-29
 * @author: geekrainy <chao.zheng02@hand-china.com>
 * @version: 0.0.1
 * @copyright: Copyright (c) 2018, Hand
 */

import { getResponse, createPagination } from 'utils/utils';
import {
  queryDocType,
  queryDocTypeDetail,
  addDocType,
  updateDocType,
  queryDocTypeAuth,
  updateDocTypeAuth,
  createDataRules,
} from '../../services/hiam/docTypeService';
import { queryIdpValue } from '../../services/api';

export default {
  namespace: 'docType',

  state: {
    docTypeList: {}, // 单据类型定义列表
    docTypeDetail: {}, // 单据类型定义详情
    docTypeAuth: [], // 单据类型权限维度
    roleAuthScopeTypeCode: [], // 权限维度列表
    docTypeLevelCode: [], // 单据权限层级编码
    pagination: {}, // 单据权限列表分页对象
  },

  effects: {
    *queryLevelCode(_, { call, put }) {
      const res = yield call(queryIdpValue, 'HIAM.DOC_TYPE_LEVEL_CODE');
      const docTypeLevelCode = getResponse(res);
      yield put({
        type: 'updateState',
        payload: { docTypeLevelCode },
      });
    },
    // 查询单据类型定义列表
    *queryDocType({ payload }, { call, put }) {
      const res = yield call(queryDocType, payload);
      const docTypeList = getResponse(res);
      const pagination = createPagination(docTypeList);

      yield put({
        type: 'updateState',
        payload: {
          docTypeList,
          pagination,
        },
      });
    },
    // 查询单据类型定义详情
    *queryDocTypeDetail({ payload }, { call, put }) {
      const res = yield call(queryDocTypeDetail, payload);
      const docTypeDetail = getResponse(res);
      yield put({
        type: 'updateState',
        payload: { docTypeDetail },
      });
    },
    // 新增或更新单据类型定义
    *saveDocType({ payload }, { call }) {
      let res;
      if (payload.docTypeId) {
        res = yield call(updateDocType, payload);
      } else {
        res = yield call(addDocType, payload);
      }
      return getResponse(res);
    },
    // 查询单据类型权限维度
    *queyDocTypeAuth({ payload }, { call, put, all }) {
      const [docTypeAuth, roleAuthScopeTypeCode] = yield all([
        call(queryDocTypeAuth, payload),
        call(queryIdpValue, 'HIAM.AUTHORITY_TYPE_CODE'),
      ]);
      const result = {
        docTypeAuth: getResponse(docTypeAuth),
        roleAuthScopeTypeCode: getResponse(roleAuthScopeTypeCode),
      };
      yield put({
        type: 'updateState',
        payload: result,
      });
      return result;
    },
    // 更新单据类型权限维度
    *updateDocTypeAuth({ payload }, { call }) {
      const res = yield call(updateDocTypeAuth, payload);
      return getResponse(res);
    },
    // 批量生成数据屏蔽规则
    *createDataRules({ payload }, { call }) {
      const res = yield call(createDataRules, payload);
      return getResponse(res);
    },
  },

  reducers: {
    updateState(state, action) {
      return {
        ...state,
        ...action.payload,
      };
    },
  },
};
