...
import {
  ...
  unBindOpenAccount,
  beforeBind,
} from '../../services/hiam/userInfoService';
...
export default {
  ...
  effects: {
    ...
    *validatePreValidate({ payload }, { call, put }) {
      ...
      const nextModalProps = { ...modalProps };
      if (captchaKey) {
        res = getResponse(yield call(userInfoValidatePre, { captchaKey, captcha }));
    ...
    *updatePassword({ payload }, { call, put }) {
      ...
      const res = getResponse(yield call(userInfoUpdatePassword, { password, originalPassword }));
      if (res) {
        yield put({
          type: 'updateState',
          payload: {
