...
import { Checkbox, Col, Form, Row, Modal, Input, Button, Table } from 'hzero-ui';
import { differenceWith, forEach, isEmpty, map, find } from 'lodash';
...
export default class UserGroupModal extends React.Component {
  ...
  renderForm() {
    const { form, userRecord = {}, isSameUser = true } = this.props;
    const {
      selectedRowKeys = [],
      dataSource = [],
      pagination = false,
    ...
    const groupNode = isSameUser ? (
      <Col key="group-same-user">
        <Form.Item
          label={intl.get('hiam.subAccount.view.message.title.group').d('用户组')}
          labelCol={{ span: 3 }}
          wrapperCol={{ span: 21 }}
        >
          <Table
      ...
      <React.Fragment key="group-no-same-user-btn">
        <Col>
          <Form.Item
            ...
            wrapperCol={{ span: 21 }}
          >
            <Button onClick={this.handleGroupCreate} style={{ marginRight: 10 }} icon="plus">
  ...
  handleGroupCreate() {
    ...
    const userId = []; // 当前编辑帐号的帐号id( 需要的排除帐号对应的用户组 )
    const excludeIds = [];
    if (!isCreate) {
