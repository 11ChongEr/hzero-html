...
import { Modal, Upload, Button, Icon } from 'hzero-ui';
import Cropper from 'react-cropper';
import 'cropperjs/dist/cropper.css';
import { Bind } from 'lodash-decorators';
import { isEmpty, isObject, isString, indexOf } from 'lodash';
...
export default class AvatarUploadModal extends React.PureComponent {
  ...
  handleImgChange(info) {
    ...
    reader.onload = function() {
      dispatch({
        ...
        payload: { uploadImgPreviewUrl: reader.result, uploadImgName: info.file.name },
      });
    };
  }
  ...
  validateUploadParams() {
    ...
    if (storageSize && flag) {
      ...
      message = flag
        ? ''
        : intl
            .get('hiam.userInfo.view.message.sizeError', {
              maxSize: `${storageSize}${storageUnit}`,
            })
            .d(`文件大小不支持，最大支持${storageSize}${storageUnit}`);
    }
  ...
  handleCutAvatar() {
    ...
    dispatch({ type: 'userInfo/updateState', payload: { imgUploadStatus: 'uploading' } });
    ...
    if (message) {
      ...
      dispatch({ type: 'userInfo/updateState', payload: { imgUploadStatus: 'waiting' } });
      return;
    }

    dispatch({
      type: 'userInfo/uploadAvatar',
      payload: { organizationId, image: imgFormData, uploadImgName, bucketName: 'public' },
    }).then(res => {
    ...
    return (
      <Modal
        ...
        footer={[
          ...
          >
            {intl.get('hzero.common.button.ok').d('保存')}
          </Button>,
        ]}
      >
        {fileList.length === 0 ? (
              ...
              <p className="ant-upload-hint">
                {typeFlag &&
                  intl
                    ...
                    .d(`支持${fileFormat}格式，`)}
                {intl
                  .get('hiam.userInfo.view.message.uploadSize', {
                    uploadSize: `${storageSize}${storageUnit}`,
                  })
                  .d(`不能大于${storageSize}${storageUnit}`)}
              </p>
            )}
          </Dragger>
        ) : (
