...
import { Icon, Input, Form, Button, Modal } from 'hzero-ui';
...
export default class SafeInfo extends React.Component {
  ...
  render() {
    ...
    const { editModalLoading = false, modalProps: editFormModalProps } = this.props;
    ...
    return (
      <div className={styles.safe}>
        ...
        <Main
          title={intl.get('hiam.userInfo.view.title.subMain.openApp').d('第三方帐号绑定')}
        ...
        <Main
          title={
            <div className={styles['login-log']}>
              {intl.get('hiam.userInfo.view.login.log').d('登录日志')}
              <Button key="bind" type="primary" onClick={this.handleCheckLoginLog}>
  ...
  handlePasswordUpdate(fieldsValue) {
    ...
    onPasswordUpdate({
      ...
      originalPassword: fieldsValue.originalPassword,
    }).then(res => {
      if (res) {
  ...
  renderPasswordFormItems(form) {
    ...
    // const { validateNewPasswordNotSame } = this; // 验证改变了 this
    return [
      <FormItem
        required
          ...
          rules: [
            {
              ...
              message: intl
                .get('hzero.common.validation.notNull', {
                  name: intl.get('hiam.userInfo.model.user.password').d('密码'),
                })
                .d('密码不能为空'),
            },
            {
      ...
      //         message: intl
      //           .get('hzero.common.validation.notNull', {
      //             name: intl.get('hiam.userInfo.model.user.anotherPassword').d('确认密码'),
      //           })
      //           .d('确认密码不能为空'),
      //       },
      //       {
    ...
    if (userInfo.phoneCheckFlag === 1) {
      content = (
        <span className={styles['color-bind']}>
          ...
          &nbsp;
          {userInfo.phone}
          {userInfo.phone ? <React.Fragment>&nbsp;</React.Fragment> : ''}
      ...
      btns.push(
        <Button key="bind" type="primary" onClick={this.handleUnCheckedPhoneBind}>
          {intl.get('view.option.bind').d('绑定')}
        </Button>
      );
    }
    return (
    ...
    if (userInfo.emailCheckFlag === 1) {
      content = (
        <span className={styles['color-bind']}>
          ...
          &nbsp;
          {userInfo.email}
          {userInfo.email ? <React.Fragment>&nbsp;</React.Fragment> : ''}
          {intl.get('hiam.userInfo.view.message.bind').d('已绑定')}
        </span>
      );
      btns.push(
    ...
    return map(openAccountList, openApp => {
      ...
      return (
        <MaxLenItem
          ...
          icon={<img src={openApp.appImage} alt="" style={{ height: 14, width: 14 }} />}
  ...
  handleBindApp(item) {
    const { dispatch } = this.props;
    dispatch({
      type: 'userInfo/beforeBind',
      payload: { appCode: item.appCode },
    }).then(res => {
      if (res) {
        window.open(res.authorizePath);
      }
    });
  }
  ...
  handleEmailEdit() {
    ...
    this.handleOpenForm({
      ...
      // 如果之前没有手机, 校验身份使用密码， 否值 默认使用 手机认证身份
      ...(phone
    ...
    const formProps = {
      ...
      onOk: (fieldsValue, form) => {
        ...
        dispatch({
          ...
          payload: { captcha, type: 'oldEmail', modalProps },
        }).then(res => {
          if (res) {
            onNext();
          }
      ...
      return (
        <React.Fragment>
          ...
          >
            ...
            })(<Input style={{ width: 143, marginRight: 20 }} />)}
            <Button
              style={{ width: 100 }}
          ...
          <FormItem wrapperCol={{ span: 22 }} style={{ textAlign: 'right' }}>
            <a
              onClick={() => {
                this.handleOpenForm({
                  ...this.getValidatePasswordFormProps(onNext),
  ...
  getValidateOldPhoneFormProps(onNext) {
    const formProps = {
      ...
      onOk: (fieldsValue, form) => {
        ...
        dispatch({
          type: 'userInfo/validatePreValidate',
          payload: { captcha, type: 'oldPhone', modalProps },
        }).then(res => {
          if (res) {
            onNext();
          }
  ...
  getValidatePasswordFormProps(onNext) {
    const formProps = {
      ...
      onOk: (fieldsValue, form) => {
        ...
        dispatch({
          type: 'userInfo/validatePreValidate',
          payload: { captcha, captchaKey: modalProps.captchaKey, modalProps },
        }).then(res => {
          if (res) {
            onNext();
          }
  ...
  getValidateUnCheckedEmailFormProps() {
    const formProps = {
      title: intl.get('hiam.userInfo.view.message.title.form.unCheckedEmail').d('绑定邮箱'),
