...
export default class DateTimeFormat extends React.Component {
  ...
  renderDateFormat() {
    ...
    let content = currentMoment.format(userInfo.dateFormat || DEFAULT_DATE_FORMAT);
    const comment = intl.get('hiam.userInfo.view.message.date').d('日期首选项，择不同的日期格式');
    const btns = [];
    if (editing) {
      // comment = '';
      content = (
        <React.Fragment>
          {form.getFieldDecorator('dateFormat', {
            initialValue: userInfo.dateFormat || DEFAULT_DATE_FORMAT,
          })(
  ...
  renderTimeFormat() {
    ...
    let content = currentMoment
      .utcOffset(timeZone2MomentUTC[userInfo.timeZone] || 8)
      .format(userInfo.timeFormat || DEFAULT_TIME_FORMAT);
    const comment = intl.get('hiam.userInfo.view.message.time').d('时间首选项，择不同的时间格式');
    const btns = [];
    if (editing) {
      // comment = '';
      content = (
        <React.Fragment>
          {form.getFieldDecorator('timeFormat', {
            initialValue: userInfo.timeFormat || DEFAULT_TIME_FORMAT,
          })(
            <Select style={itemContentStyle}>
