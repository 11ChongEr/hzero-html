...
import { Form, Button, Table, Row, Col, Modal, Input, Popconfirm } from 'hzero-ui';
...
export default class PermissionAssign extends Component {
      ...
      <Form>
        <Row>
          <Col span={8}>
            <Form.Item
              label={intl
                .get('hrpt.reportDefinition.model.reportPermission.reportCode')
                .d('报表代码')}
              {...formLayout}
            >
          ...
          <Col span={8}>
            <Form.Item
              label={intl
                .get('hrpt.reportDefinition.model.reportPermission.reportId')
                .d('报表名称')}
              {...formLayout}
            >
              {getFieldDecorator('reportId', {
                initialValue: currentReportData.reportId,
              })(<Lov textValue={currentReportData.reportName} code="HRPT.REPORT" disabled />)}
            </Form.Item>
          </Col>
          <Col span={8}>
            <Form.Item
              label={intl
                .get('hrpt.reportDefinition.model.reportPermission.reportTypeCode')
                .d('报表类型')}
              {...formLayout}
            >
        ...
        <Row>
          <Col span={8}>
            <Form.Item
              label={intl.get('hiam.subAccount.view.message.title.permission').d('权限')}
              {...formLayout}
            >
              <Button onClick={this.handleAdd} icon="plus">
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get('hrpt.reportDefinition.model.reportPermission.roleId').d('角色'),
        dataIndex: 'roleName',
      },
      {
        title: intl.get('hrpt.reportDefinition.model.reportPermission.startDate').d('有效期从'),
        width: 150,
        dataIndex: 'startDate',
        render: dateRender,
      },
      {
        title: intl.get('hrpt.reportDefinition.model.reportPermission.endDate').d('有效期至'),
        width: 150,
        dataIndex: 'endDate',
        render: dateRender,
      },
      {
        title: intl
          .get('hrpt.reportDefinition.model.reportPermission.permissionDesc')
          .d('权限说明'),
        dataIndex: 'remark',
        // width: 150,
      },
