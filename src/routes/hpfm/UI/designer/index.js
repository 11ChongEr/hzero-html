...
export default class PageDesigner extends React.Component {
  ...
  render() {
    const { saving = false, fetching = false, bankPath } = this.props;
    const { contextState } = this.state;
    return (
      <DynamicConfigContext.Provider value={contextState}>
        <Header title="页面配置" backPath={bankPath}>
          <Button type="primary" onClick={this.handleSaveBtnClick} loading={saving}>
            保存
          </Button>
          <Button onClick={this.handleGotoPreview}>预览</Button>
        </Header>
