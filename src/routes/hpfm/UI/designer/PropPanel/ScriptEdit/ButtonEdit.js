...
import { queryTplAndScriptsByPageCode } from '../../../../../../services/hpfm/uiPageService';
  ...
  getValidateLevel2PageData(validateData, attrConfig) {
    ...
    if (editSubEvents && editSubEvents.length) {
      ...
      for (let i = 0; i < validateSubEvents.length; i += 1) {
        ...
        subEvents.push({
          attributeType: DataType.String,
          value: `${validateSubEvents[i].subEventListen}${subEventSep}${
            validateSubEvents[i].subEventAction
          }`,
        });
    ...
    if (editModalBtns && editModalBtns.length) {
      ...
      for (let i = 0; i < validateModalBtns.length; i += 1) {
        ...
        modalBtns.push({
          attributeType: DataType.String,
          value: `${validateModalBtns[i].btnName}${modalBtnSep}${
            validateModalBtns[i].btnType
          }${modalBtnSep}${validateModalBtns[i].btnAction}`,
        });
