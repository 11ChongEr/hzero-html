...
export default class ListTable extends Component {
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get(`${promptCode}.model.dashboard.enabledFlag`).d('状态'),
      ...
      },
      {
        ...
        render: (_, record) => {
          const actions = [
            <a key="edit" onClick={() => this.showEditModal(record)}>
              {intl.get('hzero.common.button.edit').d('编辑')}
            </a>,
          ];
          if (!isTenantRoleLevel() && record.dataTenantLevel === 'TENANT') {
            actions.push(
              <a key="assignCard" onClick={() => this.handleLineAssignCardBtnClick(record)}>
