...
import { Form, Button, Tabs } from 'hzero-ui';
...
@connect(({ employee, loading }) => ({
  employee,
  loading: {
    position: loading.effects['employee/fetchPosition'],
    user: loading.effects['employee/fetchUser'],
    save: loading.effects['employee/saveData'] || loading.effects['employee/fetchDetail'],
    searchUser: loading.effects['employee/searchUser'],
    searchPosition: loading.effects['employee/searchPosition'],
  },
...
export default class Detail extends Component {
  ...
  handleChangeTree(record) {
    ...
    const newTree = this.findAndSetNodeProps(positions, pathMap[record.id], record);
    dispatch({
      type: 'employee/updateState',
      payload: {
        positions: newTree,
  ...
  render() {
    ...
    const { form, employee, loading } = this.props;
    const {
      employeeInfo = {},
      positionList = [],
      userList = [],
    ...
    const positionModalProps = {
      expandedRowKeys,
      loading: loading.searchPosition,
    ...
    return (
      <Fragment>
        ...
        >
          <Button icon="save" type="primary" onClick={this.handleSave} loading={loading.save}>
