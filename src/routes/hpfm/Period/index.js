...
@connect(({ period, loading }) => ({
  period,
  loading: {
    search: loading.effects['period/searchPeriodHeader'],
    savePeriod: loading.effects['period/savePeriod'],
    save: loading.effects['period/savePeriodHeader'],
    searchLine: loading.effects['period/searchPeriodLine'],
  },
}))
      ...
      <Fragment>
        <Header title={intl.get('hpfm.period.view.message.tab.define').d('期间定义')}>
          <Button
            ...
            style={{ display }}
            loading={loading.save || loading.search}
          >
            {intl.get('hzero.common.button.save').d('保存')}
          </Button>
          <Button icon="plus" onClick={this.handleAddPeriodHeader} style={{ display }}>
            {intl.get('hzero.common.button.create').d('新建')}
          </Button>
        </Header>
        <Content>
          <Tabs defaultActiveKey="define" type="card" onChange={this.handleChangeTag}>
            <Tabs.TabPane
