...
const viewMessagePrompt = 'hitf.application.view.message';
...
export default class Editor extends PureComponent {
  ...
  render() {
    ...
    const editable = isNumber(applicationId);
    const title = editable
      ? intl
          .get(`${viewMessagePrompt}.title.editor.withEditedName`, {
            name: formDataSource.applicationName || '',
          })
          .d(`修改“${formDataSource.applicationName || ''}”`)
      : intl.get(`${viewMessagePrompt}.title.editor.create`).d('创建应用');
    const drawerProps = {
      title,
      visible,
      mask: true,
