...
export default class ServiceEditor extends PureComponent {
  ...
  getSnapshotBeforeUpdate(prevProps) {
    ...
    return (
      visible && isNumber(interfaceServerId) && interfaceServerId !== prevProps.interfaceServerId
    );
  }
  ...
  handleChangeListTenant(value) {
    ...
    const newDataSource = interfaceListDataSource.map(item => {
  ...
  render() {
    ...
    const title = isNumber(interfaceServerId)
      ? intl.get(`${viewMessagePrompt}.title.editor.edit`).d('修改服务配置')
      : intl.get(`${viewMessagePrompt}.title.editor.create`).d('注册服务');
    const drawerProps = {
      title,
      visible,
      mask: true,
      ...
      <Drawer {...drawerProps}>
        ...
        <div className={styles['hiam-interface-model-btns']}>
          ...
          {editable ? (
            ...
            >
              {intl.get(`${viewButtonPrompt}.create`).d('注册')}
            </Button>
          )}
        </div>
