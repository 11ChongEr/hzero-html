...
export default class EmployeeDrawer extends PureComponent {
  ...
  handleSubmit() {
    ...
    const { delegateOrAddUser, isCarbon, processCarbonCopy, taskAction } = this.props;
    if (isCarbon) {
      processCarbonCopy();
    } else if (employeeNum) {
      taskAction({ action: delegateOrAddUser, targetUser: employeeNum, targetUserName: name });
    } else {
      Modal.warning({
