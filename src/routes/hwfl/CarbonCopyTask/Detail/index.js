...
@connect(({ carbonCopyTask, loading }) => ({
  ...
  fetchForecast: loading.effects['carbonCopyTask/fetchForecast'],
  tenantId: getCurrentOrganizationId(),
}))
@formatterCollections({
  code: ['hwfl.carbonCopyTask', 'hwfl.common', 'entity.position', 'entity.department'],
})
...
export default class Detail extends Component {
  ...
  executeTaskAction() {
    ...
    Modal.confirm({
      ...
      content: intl.get('hwfl.carbonCopyTask.view.message.title.confirmBack').d(`确认撤回吗?`),
      onOk: () => {
        const params = {
        ...
        dispatch(params).then(res => {
          if (res) {
            ...
            closeTab(`/hwfl/workflow/carbon-copy-task/detail/${match.params.id}`);
            this.props.history.push(`/hwfl/workflow/carbon-copy-task`);
          }
        });
      },
    });
    ...
    return (
      <Fragment>
        <Header
          ...
          // backPath="/hwfl/workflow/carbon-copy-task/"
        >
