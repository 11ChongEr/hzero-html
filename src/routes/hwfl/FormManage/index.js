 ...
 */
@formatterCollections({ code: ['hwfl.formManage', 'hwfl.common'] })
...
export default class FormManage extends Component {
    ...
    dispatch({
      ...
      if (res && res.failed && codeValue) {
        if (res.code === 'error.process.form.definition.code.exist') {
          ...
          callback(intl.get('hwfl.formManage.view.validation.codeNotValid').d('编码不能为纯数字'));
        }
      }
      callback();
    });
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Header title={intl.get('hwfl.formManage.view.message.title').d('表单管理')}>
          <Button
            type="primary"
            icon="plus"
            onClick={() => {
