    ...
    return (
      ...
      >
        <Collapse accordion bordered={false} onChange={onChangePanel} defaultActiveKey="deploy">
          <Collapse.Panel
            header={intl.get('hwfl.processDefine.view.message.processInfo').d('流程信息')}
            key="process"
          >
            <Form.Item
              label={intl.get('hwfl.processDefine.model.deploy.name').d('名称')}
              {...formLayout}
            >
              <Input disabled value={process.name} />
            </Form.Item>
            <Form.Item
              label={intl.get('hwfl.processDefine.model.deploy.category').d('分类')}
              {...formLayout}
            >
              <Input disabled value={process.category} />
            </Form.Item>
            <Form.Item
              label={intl.get('hwfl.processDefine.model.deploy.flag').d('标识')}
              {...formLayout}
            >
              <Input disabled value={process.flag} />
            </Form.Item>
            <Form.Item
              label={intl.get('hwfl.processDefine.model.deploy.version').d('版本')}
              {...formLayout}
            >
          ...
          <Collapse.Panel
            header={intl.get('hwfl.processDefine.view.message.deployInfo').d('部署信息')}
            key="deploy"
          >
            <Form.Item
              label={intl.get('hwfl.processDefine.model.deploy.name').d('名称')}
            ...
            <Form.Item
              label={intl.get('hwfl.processDefine.model.deploy.time').d('发布时间')}
              {...formLayout}
            >
          ...
          <Collapse.Panel
            header={intl.get('hwfl.processDefine.view.message.preview').d('预览图')}
            key="image"
          >
            <img
              alt=""
