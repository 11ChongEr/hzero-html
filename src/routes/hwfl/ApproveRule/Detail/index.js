 ...
 * @reactProps {!Object} approveRule - 数据源
...
@connect(({ approveRule, loading }) => ({
  approveRule,
  fetchApproveHeaderLoading: loading.effects['approveRule/fetchApproveHeader'],
  saving:
    loading.effects['approveRule/updateHeader'] || loading.effects['approveRule/createHeader'],
...
export default class Detail extends Component {
  ...
  handleSearch() {
    ...
    if (!isUndefined(id)) {
      dispatch({
        type: 'approveRule/fetchApproveHeader',
        payload: {
          tenantId,
    ...
    dispatch({
      type: 'approveRule/fetchParamsType',
    });
    dispatch({
      type: 'approveRule/fetchInterfaceMap',
      payload: { tenantId },
    });
    dispatch({
      type: 'approveRule/fetchReturnType',
    });
  }
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        if (isUndefined(match.params.id)) {
          dispatch({
            type: 'approveRule/createHeader', // 新增逻辑
            payload: {
              tenantId,
              dto: { parameters: [...anotherParameters], ...values, type: 'approve' }, // type指定为approve
            },
          }).then(res => {
            if (res) {
              ...
              dispatch(
                routerRedux.push({
                  pathname: `/hwfl/setting/approve-rule/detail/${res.externalDefinitionId}`,
                })
              );
            }
          });
          ...
          dispatch({
            type: 'approveRule/updateHeader', // 更新逻辑
            payload: {
              tenantId,
              externalDefinitionId: header.externalDefinitionId,
              dto: { ...header, parameters: [...anotherParameters], ...values, type: 'approve' },
            },
          }).then(res => {
            if (res) {
              notification.success();
  ...
  handleSaveContent(values) {
    ...
    dispatch({
      type: 'approveRule/updateState',
      payload: {
        header: { parameters: [...parameters, value], ...otherValues },
      },
    });
  ...
  render() {
    ...
    const headerTitle = isUndefined(match.params.id)
      ? intl.get('hwfl.approveRule.view.message.title.add').d('审批规则 - 新建')
      : intl.get('hwfl.approveRule.view.message.title.edit').d('审批规则 - 编辑');
    ...
    const listProps = {
      paramsTypeList,
      loading: fetchApproveHeaderLoading,
    ...
    return (
      <React.Fragment>
        <Header title={headerTitle} backPath="/hwfl/setting/approve-rule/list">
        ...
        <Content>
          <Form>
            <Row gutter={24} type="flex">
              ...
              <Col span={8}>
                <Form.Item
                  label={intl.get('hwfl.approveRule.model.approveRule.scope').d('数据范围')}
                  {...formLayout}
                >
