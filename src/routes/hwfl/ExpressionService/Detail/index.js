/**
 * ExpressionService/Detail - 流程设置/表达式服务管理
 * @date: 2018-8-23
 ...
 */
@formatterCollections({ code: ['hwfl.expressionService', 'hwfl.common'] })
@Form.create({ fieldNameProp: null })
@connect(({ expressionService, loading }) => ({
  expressionService,
  loading: loading.effects['expressionService/fetchApproveHeader'],
...
export default class Detail extends Component {
  ...
  handleSearch() {
    ...
    if (!isUndefined(id)) {
      dispatch({
        type: 'expressionService/fetchApproveHeader',
        payload: {
          tenantId,
      ...
      dispatch({
        type: 'expressionService/updateState',
        payload: {
          header: {},
        },
      });
    ...
    dispatch({
      type: 'expressionService/fetchParamsType',
    });
    dispatch({
      type: 'expressionService/queryCategory',
      payload: { tenantId },
    });
    dispatch({
      type: 'expressionService/fetchInterfaceMap',
      payload: { tenantId },
    });
    dispatch({
      type: 'expressionService/fetchReturnType',
    });
  }
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        if (isUndefined(match.params.id)) {
          dispatch({
            type: 'expressionService/saveHeader', // 新增逻辑
            payload: {
              tenantId,
              dto: { parameters: [...anotherParameters], ...values, type: 'expressionService' }, // type指定为approve
            },
          }).then(res => {
            if (res) {
              ...
              dispatch(
                routerRedux.push({
                  pathname: `/hwfl/setting/expression-service/detail/${res.externalDefinitionId}`,
                })
              );
            }
          });
          ...
          dispatch({
            type: 'expressionService/updateHeader', // 更新逻辑
            payload: {
              tenantId,
  ...
  handleDeleteContent(record) {
    ...
    if (isNumber(recordParameterId)) {
      dispatch({
        type: 'expressionService/deleteLine',
  ...
  checkUnique(rule, value, callback) {
    ...
    dispatch({
      type: 'expressionService/checkUnique',
      payload: {
        tenantId,
        code: value,
        type: 'expressionService',
  ...
  render() {
    ...
    const headerTitle = isUndefined(match.params.id)
      ? intl.get('hwfl.expressionService.view.message.title.add').d('表达式服务 - 新建')
      : intl.get('hwfl.expressionService.view.message.title.edit').d('表达式服务 - 编辑');
    ...
    return (
      <React.Fragment>
        <Header title={headerTitle} backPath="/hwfl/setting/expression-service/list">
