...
@formatterCollections({ code: ['hwfl.delegate'] })
...
export default class Delegate extends Component {
    ...
    if (timeCorrect) {
      if (startOrEnd === 'start') {
        params = {
          errorFlag: true,
          errorMessage: intl
            .get('hwfl.delegate.view.message.startIsBefore')
            .d('转交开始日期不能早于当前时间'),
        };
        ...
        params = {
          errorFlag: true,
          errorMessage: intl
            .get('hwfl.delegate.view.message.endIsBefore')
            .d('转交截止日期不能早于当前时间'),
        };
        this.setState({ validateData: params });
      }
  ...
  compareDateTwo(endDate, startDate) {
    ...
    if (timeCorrect) {
      params = {
        errorFlag: true,
        errorMessage: intl
          .get('hwfl.delegate.view.message.isBefore')
          .d('转交截止日期不能早于转交开始日期'),
      };
      this.setState({ validateData: params });
      return false;
  ...
  renderForm() {
    ...
    return (
      <Form style={{ width: '500px' }}>
        <FormItem
          label={intl.get('hwfl.delegate.view.message.delegateStartDate').d('转交开始日期')}
          {...formItemLayout}
        >
        ...
        <FormItem
          label={intl.get('hwfl.delegate.view.message.delegateEndDate').d('转交截止日期')}
          {...formItemLayout}
        >
        ...
        <FormItem
          label={intl.get('hwfl.delegate.view.message.delegate').d('转交人')}
          {...formItemLayout}
        >
  ...
  render() {
    return (
      <React.Fragment>
        <Header title={intl.get('hwfl.delegate.view.message.title').d('自动转交配置')}>
          <Button
            icon="save"
            type="primary"
            htmlType="submit"
