...
import { Form, Button, Table, Row, Col, Modal, Input } from 'hzero-ui';
...
export default class PermissionAssign extends Component {
  ...
  render() {
    ...
    const title = targetItem.permissionId
      ? intl.get('hrpt.concPermission.view.message.permission.edit').d('编辑权限')
      : intl.get('hrpt.concPermission.view.message.permission.add').d('新增权限');
    const drawerProps = {
      title,
      currentTenantId,
      tenantRoleLevel,
    ...
    const columns = [
      ...
      {
        title: intl.get('hrpt.concPermission.model.concPermission.roleId').d('角色'),
        dataIndex: 'roleName',
      },
      {
        title: intl.get('hrpt.concPermission.model.concPermission.startDate').d('有效期从'),
        width: 150,
        dataIndex: 'startDate',
        render: dateRender,
      },
      {
        title: intl.get('hrpt.concPermission.model.concPermission.endDate').d('有效期至'),
        width: 150,
        dataIndex: 'endDate',
        render: dateRender,
      },
    ...
    return (
      <Modal
        title={intl.get('hrpt.concPermission.modal.concPermission.assignPermission').d('分配权限')}
