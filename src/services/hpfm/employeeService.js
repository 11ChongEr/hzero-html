...
export async function saveEmployee(params) {
  const { tenantId, saveData, positionId } = params;
...
export async function searchEmployeeById(params) {
  const param = parseParameters(params);
  return request(`${prefix}/${param.tenantId}/employees/ids`, {
    method: `GET`,
    query: {
      employeeIds: param.employeeId,
    },
  });
}
...
export function renderTreeData(collections = [], levelPath = {}) {
  ...
  collections.map(item => {
    const temp = item;
    const parentPath = pathMap[temp.parentId] || pathMap[temp.unitId] || [];
    pathMap[temp.id] = [...parentPath, temp.id];
    if (item.children) {
      renderTreeData(temp.children, pathMap);
    }
    return pathMap;
  });
  return pathMap;
