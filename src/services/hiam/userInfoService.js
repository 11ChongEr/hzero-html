...
import { HZERO_IAM, HZERO_PLATFORM, HZERO_FILE } from 'utils/config';
...
export async function userInfoUpdateDefaultRole(organizationId, roleId) {
  return request(`${HZERO_IAM}/hzero/v1/${organizationId}/users/default-role`, {
    method: 'PUT',
    query: {
      roleId,
...
export async function userInfoUpdateDefaultCompany(organizationId, companyId) {
  return request(`${HZERO_IAM}/hzero/v1/${organizationId}/users/default-company`, {
    method: 'PUT',
    query: {
      companyId,
      tenantId: organizationId,
