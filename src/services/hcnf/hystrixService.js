import { HZERO_PLATFORM, HZERO_HCNF } from 'utils/config';
...
export async function fetchList(params = {}) {
  return request(`${HZERO_HCNF}/v1/${hystrixApi()}`, {
    method: 'GET',
...
export async function fetchHeaderInformation(params) {
  ...
  return request(`${HZERO_HCNF}/v1/${hystrixApi()}/${confId}`, {
    method: 'GET',
    query: { page, pageSize },
  });
}
...
export async function refresh(params) {
  return request(`${HZERO_HCNF}/v1/${hystrixApi()}/batch-refresh`, {
    method: 'POST',
    body: params,
  });
}
...
export async function fetchDetailList(params = {}) {
  return request(`${HZERO_HCNF}/v1/${hystrixLineApi()}`, {
    method: 'GET',
    query: params,
  });
}
