...
export default class CustomTable extends React.Component {
  ...
  render() {
    ...
    const filterBarLocale = {
      display: intl.get('hpfm.common.userUI.view.filterBar.display').d('显示'),
      field: intl.get('hpfm.common.userUI.view.filterBar.field').d('字段'),
      fixedLeft: intl.get('hpfm.common.userUI.view.filterBar.lock').d('lock'),
      orderSeq: intl.get('hpfm.common.userUI.view.filterBar.orderSeq').d('排序'),
    };
    return init ? (
      <Table
  ...
  async loadHiddenColumns(code) {
    ...
    try {
      const customColumns = await request(`${HZERO_PLATFORM}/v1/${tenantId}/custom/table/${code}`);
  ...
  onColumnFilter = (customColumns = []) => {
    ...
    if (customColumns && customColumns.length > 0) {
      ...
      return request(`${HZERO_PLATFORM}/v1/${tenantId}/custom/table/${customCode}`, {
        method: 'POST',
        body: customColumns,
      })
        .then(res => {
  ...
  calculateCustomColumns(tenantCustomColumns = [], userCustomColumns = []) {
    ...
    const tableColumns = columns.filter((column, index) => {
      // 只要 hiddenByTenantFields 中有 就 不出现在新的 TableColumns 中
      return !hiddenByTenantFields.some(hiddenFieldKey => {
