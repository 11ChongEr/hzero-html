...
import { isFunction, isEmpty, map, omit, forEach, set, split } from 'lodash';
...
import { paramTypeCode, actionCode, openTypeCode, openPageModalBodyStyle } from './utils';
import {
  dealObjectProps,
...
import { fieldLabelProp, fieldNameProp, dynamicTableOmitProps, subEventSep } from '../config';
  ...
  handleLineRemove(removeRecord) {
    ...
    if (removeUrl) {
      ...
      notification.warning({
        message: '非法的调用, 请联系管理员',
      });
      return Promise.reject(new Error('非法的调用, 请联系管理员'));
    } else if (batchRemoveUrl) {
      if (removeRecord) {
        return fetchDelete({
          url: batchRemoveUrl,
          body: [removeRecord],
    ...
    this.hiddenLoading('lineRemoveLoading');
    return Promise.reject(new Error('表格没有提供 删除URL 或 批量删除URL'));
  }
  ...
  remove() {
    ...
    if (removeUrl) {
      ...
      if (selectedRows && selectedRows.length > 0) {
        ...
        return Promise.reject(new Error('没有选中要删除的数据'));
      }
    }
    this.hiddenLoading('removeLoading');
    return Promise.reject(new Error('表格没有提供批量删除URL'));
  }
  ...
  getColumnsAndScrollX() {
    ...
    const columns = map(fields, field => {
      ...
      const column = {
        ...
        render: (_, record) => {
          return getDisplayValue({ field, dataSource: record, componentProps });
        },
      };
      if (isAutoWidth) {
        hasAutoWidth = true;
          ...
          column.render = (_, record) => {
            ...
            forEach(btns, btnProps => {
              ...
              linkBtns[btnProps.orderSeq] = (
                <a key={btnKey} onClick={onBtnClick} style={linkButtonStyle}>
                  {btnProps.title}
                </a>
              );
            });
            return <span className="action-link">{linkBtns}</span>;
          };
          break;
        default:
          break;
